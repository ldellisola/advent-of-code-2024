...
public class RobotHandler(string input)
{
    ...
    public long CalculateCodeComplexity()
    {
        ...
        string[] intermediates = new string[ 26 ];
        intermediates[ 0 ] = robot1;
        for (int i = 1; i <= 25; i++)
            intermediates[i] = ControlDirectionalPad(intermediates[i - 1]);
        
        // robot2 = ControlDirectionalPad(robot1);
        instructions = ControlDirectionalPad(intermediates[25]);
    ...
    private Dictionary<(Vector, char[]), (Vector,char[])> Cache = [];

    private  (Vector,char[]) Test(Vector position, char[] code, int depth)
    {
        if (depth == 26)
            throw new Exception("Nopeee");
        
        if (Cache.TryGetValue((position, code), out var value))
            return value;

        Vector downstreamPosition = (0, 2);
        var allChars = new List<char>();
        foreach(var character in code)
        {
            var nextPosition = CharToDirectionPadPosition(character);
            if(depth < 25)
            {
                var a = Test(downstreamPosition, GenerateDirections(position, nextPosition), depth + 1);
                
                // Cache.Add((position,));
                downstreamPosition = a.Item1;
                allChars.AddRange(a.Item2);
            }
            position = nextPosition;
        }

        return (position, allChars.ToArray());
    }

    private char[] GenerateDirections(Vector position, Vector nextPosition)
    {
        ...
        if(position == CharToDirectionPadPosition('A') && nextPosition == CharToDirectionPadPosition('<'))
        {
            return ['v','<','<','A'];
        }
        if(position == CharToDirectionPadPosition('<') && nextPosition == CharToDirectionPadPosition('^'))
        ...
        if(position == CharToDirectionPadPosition('<') && nextPosition == CharToDirectionPadPosition('A'))
        {
            return ['>', '>', '^','A'];
        }
        ...
        while(position != nextPosition)
        {
            ...
            chars.Add(FromDirection(direction));
            position += direction;
            if(position == (0, 0))
